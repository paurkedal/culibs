 ...
 ** nodes for best efficiency.  The slots are accessed directly using the
 ** <code>*_mem</code> functions, whereas some specialised functions are
 ** provided for pointers and integers.
 **
 ** \see cucon_pset_h
 ** \see cucon_umap_h */
...
/** If \a key is not in \a map, this call allocates a node of size \a
 ** node_size, initialises a \c cucon_pmap_node from offset 0 with the key \a
 ** key, inserts the node into \a map, assigns it to <code>*\a node_out</code>,
