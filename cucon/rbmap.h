...
/** A sorted map implemented as a red-black tree. */
struct cucon_rbmap
{
...
/** Construct \a map as an empty map where keps are totally ordered according
 ** to \a cmp, where <code>\a cmp(\a x, \a y)</code> shall return
 ** - negative to indicate that \e x precedes \e y
 ** - positive to indicate that \e x succeeds \e y
 ** - zero to indicate that \e x = \e y. */
void cucon_rbmap_init(cucon_rbmap_t map, cu_clop(cmp, int, void *, void *));

/** Return a map where keys are ordered according to \a cmp.
 ** \see cucon_rbmap_init for the meaning of \a cmp. */
...
/** Return the pointer value of the slot of \a key in \a map, or \c NULL if not
 ** found. */
void *cucon_rbmap_find_ptr(cucon_rbmap_t map, void *key);

/** If \a key is not in \a map, then insert it with an associated slot of \a
 ** slot_size bytes.  Returns true iff \a key was inserted.  In any case,
 ** return the slot in \c *\a slot. */
cu_bool_t cucon_rbmap_insert_mem(cucon_rbmap_t map, void *key,
			         size_t slot_size, cu_ptr_ptr_t slot);

/** If \a key is not in \a map, then insert it with a pointer-valued slot set
 ** to \a val.  Returns true iff \a key was inserted. */
...
/** Sequentially conjunct \a cb over (key, slotptr)-pairs of \a map in order as
 ** given by the ordering operator of \a map. */
