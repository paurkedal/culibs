 ...
 ** and generated by expressions which are explicitly tagged with the monoid
 ** operator.
 **
 ** Compared to \ref cuex_monoid_h "cuex/monoid.h", tagging the generators
 ** makes this variant suitable for dealing with nested elements of the same
 ** monoid, where the tag protects against the associativity.  In other words,
 ** it can serve as tuples of arbitrary arity (including 0 and 1) with tuple
...
/** Returns \a x * \a y where * = \a mult where \a x and \a y must be members
 ** of the tagged monoid under \a mult. */
