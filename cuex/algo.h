...
/** Returns <code>cu_call(\a f, \a e)</code> if non-\c NULL, else if \a e is
 ** an operation, returns it with each operand transformed by a recursive call
 ** to this function, else (if \a e is not and operation) returns \a e.  This
 ** function handles transformations of ACI trees.
 **
 ** This is quite generally applicable for doing deep transformations.  It is
 ** based on the idea that \a f will either know how to handle a certain
...
/** Sequentially conjunct \a f over variables in \a e in depth-first order. */
cu_bool_t cuex_depth_conj_vars(cuex_t e, cu_clop(f, cu_bool_t, cuex_var_t));

/** Sequentially conjunct \a f over all nodes in \a e in depth-first
 ** leaf-to-root order. */
cu_bool_t cuex_depthout_conj(cuex_t e, cu_clop(f, cu_bool_t, cuex_t));

/** Transform each node of \a e with \a f in depth-first leaf-to-root
 ** order. */
cuex_t cuex_depthout_tran(cuex_t e, cu_clop(f, cuex_t, cuex_t));

/** Transform the leaves of \a e with \a f in depth-first order. */
cuex_t cuex_depth_tran_leaves(cuex_t e, cu_clop(f, cuex_t, cuex_t));

/** Transform the variables of \a e with \a f in depth-first order. */
cuex_t cuex_depth_tran_vars(cuex_t e, cu_clop(f, cuex_t, cuex_var_t));

/** Call \a out with the maximum subtrees of \a e for which \a pred holds true
 ** on all leaves. */
cu_bool_t cuex_maxtrees_of_leaftest_iter(cuex_t e,
					 cu_clop(pred, cu_bool_t, cuex_t leaf),
					 cu_clop(out, void, cuex_t subex));

/** Return the result of transforming with \a tran all maximum subtrees of \a e
 ** for which \a pred holds true on all leaves.
 ...
 ** <code>unify(\a e0sub, \a e1sub)</code>.  Typical use is for \a unify to
 ** return a new variable, and possibly record the mismatching
 ** sub-expressions. */
...
/** Return the result of substituting \a var with \a value in \a e. */
cuex_t cuex_substitute_ex(cuex_t e, cuex_t var, cuex_t value);

/** Return the result of substituting in \a e all keys of \a pmap with their
 ** values, where \a pmap maps from \ref cuex_t to \ref cuex_t */
cuex_t cuex_substitute_pmap(cuex_t e, cucon_pmap_t pmap);

/** Return the leftmost leaf of \a e with the meta \a meta. */
cuex_t cuex_leftmost_with_meta(cuex_t e, cuex_meta_t meta);

/** Return the rightmost leaf of \a e with the meta \a meta. */
cuex_t cuex_rightmost_with_meta(cuex_t e, cuex_meta_t meta);

/** The leftmost identifier in \a e.
 ** \pre \a e must not contain ACI operators. */
CU_SINLINE cu_idr_t cuex_leftmost_idr(cuex_t e)
...
{ return (cu_idr_t)cuex_rightmost_with_meta(e,
				cuoo_type_to_meta(cu_idr_type())); }

/** The leftmost variable in \a e having quantification in \a qcset.
 ** \pre \a e must not contain ACI operators. */
...
/** Do a sequential conjunction of \a fn over the free variables in \a e
 ...
 ** qcset with their mapping under \a f.  \a excl is the current set of
 ** variable considered bound; it's modified in-place and restored. */
cuex_t
 ...
 ** <code>cuex_opn(\a opr, \e v, vars ∖ * {\e v})</code> for some \e v in \a
 ** vars.
 ** \pre \a opr must be binary. */
cuex_t cuex_outmost_quantify_vars(cuex_meta_t opr, cucon_pset_t vars, cuex_t e);

/* Insert e into accum and return true. */
cu_clos_edec(cuex_pset_curried_insert_ex,
	     cu_prot(cu_bool_t, cuex_t e),
    ( cucon_pset_t accu; ));

/* Erase e from accum and return true. */
