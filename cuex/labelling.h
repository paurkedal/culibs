 ...
 ** already contains the key \a l, then \a L is returned. */
cuex_t cuex_labelling_insert(cuex_t L, cuex_t l, cuex_t e);

/** Assuming \a va refers to an argument list which contains a altering
 ** sequence of pairs of labels and values terminated by \c NULL, returns the
 ** result of inserting a mapping for each pair into \a L. */
 ...
 ** present in \a L, the existing value \e v is replaced with <code>merge(\e v,
 ** \a e)</code>. */
...
/** Returns the index of \a l in \a L, or \c (size_t)-1 if not found.  Note
 ** that the complexity of this call is linear in the cardinality of \a L. */
size_t cuex_labelling_find_index(cuex_t L, cuex_t l);

/** Returns the result of erasing the mapping from \a l in \a L if present,
 ** otherwise returns \a L. */
cuex_t cuex_labelling_erase(cuex_t L, cuex_t l);

/** If \a l has a mapping if \c *\a L, updates <code>*\a L</code> by erasing
 ** it, and returns the value of the mapping, otherwise returns \c NULL. */
...
/** Returns the union of \a L0 and \a L1 if the labellings are disjoint, \c
 ** NULL otherwise. */
cuex_t cuex_labelling_disjoint_union(cuex_t L0, cuex_t L1);

/** Forms the intersection of \a L0 and \a L1 considering elements equal if
 ** they have the same label.  The elements of \a L0 are used in the result. */
...
/** A \ref cu_ptr_seq_h "sinktor" for construction a new labelling by unordered
 ** insertion of \c CUEX_O2_METAPAIR nodes and labellings. */
cu_ptr_sinktor_t cuex_labelling_comm_build_sinktor(void);

/** A \ref cu_ptr_seq_h "sinktor" for extending a labelling by unordered
 ** insertion of \c CUEX_O2_METAPAIR nodes and labellings. */
