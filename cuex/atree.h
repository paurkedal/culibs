 ...
 **
 ** The internal structure of the tree is hidden.  The leafs of the tree are
 ** the inserted expressions.
 ** Most of the functions below takes a \a get_key argument, which when passed
 ** a leaf node shall compute a \c cu_word_t typed key.
 ...
 ** implement a map, use a binary operation as leafs, and cast the first
 ** operand to \c cu_word_t.
 ...
 ** <code>cu_call(get_key, e)</code> equals \a find_key.  Returns \c NULL if no
 ** such element exists.  */
cuex_t
 ...
 ** tree, replace it with <code>\a merge_values(\e e, \a value)</code>. */
cuex_t
...
/** If \c *\a tree has a node where \a get_key returns \a erase_key, updates
 ** <code>*\a tree</code> by removing this node and returns the node, otherwise
 ** returns \c NULL.  */
cuex_t
 ...
 ** and \a tree1 coincides on any of their keys. */
cuex_t
...
/** Returns the intersection of \a tree0 and \a tree1, considering two elements
 ** equal if the corresponding values returned by \a get_key are equal.  The
 ** nodes from \a tree0 are used in the result. */
cuex_t
cuex_atree_left_isecn(cu_clop(get_key, cu_word_t, cuex_t),
		      cuex_t tree0, cuex_t tree1);

/** As \ref cuex_atree_left_isecn, but merge intersecting elements from \a
 ** tree0 and \a tree1 using \a merge_values. */
cuex_t
 ...
 ** of (\a tree0, \a tree1) with equal keys, return false unless <code>\a
 ** value_subseteq(\e e0, \e e1)</code> returns true. */
cu_bool_t
...
/** Returns the image of \a tree under \a f.  Note that \a get_key is only used
 ** for building the result, and may in fact be different from the \a get_key
 ** which was used to build \a tree (thus its order in the argument list).  If
 ** \a f leaves the key of its argument unchanged, see \ref
 ...
 ** used to build the container \ref cuex_atree_insert, so the condition is
 ** that \e get_key(\a f(\e e)) = \e get_key(\e e) for all elements \e e in \a
 ...
 ** @{
 ** The following can be used to implement iterators for expression types
 ** defined in terms of associated trees. */

/** The size required by an iterator over \a tree. */
size_t cuex_atree_itr_size(cuex_t tree);

/** Initialise an \a itr as an iterator over all elements of \a tree, where \a
 ** itr points to memory of at least <code>\ref cuex_atree_itr_size(\a
 ** tree)</code> bytes. */
void cuex_atree_itr_init(void *itr, cuex_t tree);

/** Pop off and return the next subtree from \a itr, or return \c NULL if \a
 ** itr is empty. */
cuex_t cuex_atree_itr_get(void *itr);

/** Pop off the next \e subtree from \a itr and return <code>cucon_opn_at(\e
 ** subtree, 1)</code>, or return \c NULL if \a itr is empty. */
cuex_t cuex_atree_itr_get_at_1(void *itr);


/** @}
 ** \name Specialisations Acting on a Certain Operand of the Members
 ** @{
 ** The following functions are specialised variants of the corresponding
 ** functions from the primary API, where the leafs are assumed to be
 ** operations of sufficient arity, and callbacks acting on leaves are replaced
 ** by callbacks acting on a given operand of the leaves.  The caller is
 ** responsible of ensuring that all leafs have the proper form.
 **/
...
/** As \ref cuex_atree_deep_order, but only pass operand \a value_index of the
 ** leaves to the ordering functions. */
cu_order_t
 ...
 ** operations of arity at least \a value_index + 1, and transforms only
 ** operand \a value_index of the elements.  This specialisation applies to
 ** map-like types with inert keys, such as \ref cuex_labelling_h "labellings".
 **/
...
/** @}
 ** \name Specialisations for Key-Value Formed Members
 ** @{
 ** The following functions are specialised variants of the corresponding
 ** functions from the primary API, where the leafs are assumed to be binary
 ** operations where the first operand serves as a key and the second operand
 ** serves as a value.  Callbacks generally receive the key as the first
 ** argument and one or two values as the next arguments.
 **/
...
/** As \ref cuex_atree_deep_isecn, except it assumes the first operand of
 ** leaves are keys and the second oprand are values.  \a merge receives the
 ** common keys and the two values to be merged. */
cuex_t
 ...
 ** operations of arity at least 2, and transforms the second operand.  Both
 ** the first and second operands are passed to \a f in order.  This is suited
 ** for map-like types where the first operand is an inert key and the second
